...
const ViewProduct = (props) => {
    ...
    const onFinish = (values) => {
        ...
        axios.put(
            `http://${process.env.REACT_APP_API}/product/${props.product._id}`, newProduct[0]
        ).then(res => dispatch(EditProduct(props.product._id, res))).catch(err => console.log(err))
    }
    
    const onDelete = (id) => {
        axios.delete(
            `http://${process.env.REACT_APP_API}/product/${id}`
...
const Products = () => {
        ...
        const onFinish = values => {
            ...
            axios.post(`http://${process.env.REACT_APP_API}/products`, newProduct[0]).then(res => dispatch(AddProduct(res))).catch(err => console.log(err))
            form.resetFields();
        };
